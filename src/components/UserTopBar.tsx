import { Search, Menu, User, LogOut, ChevronDown, Bot, ShoppingBag, Bell, Settings } from 'lucide-react';
...
type Page = 'dashboard' | 'tickets' | 'shop' | 'assistant' | 'activity' | 'billing' | 'profile' | 'settings';
...
export function UserTopBar({ onMenuClick, onNavigate, onLogout }: UserTopBarProps) {
  const handleLogout = () => {
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="flex items-center gap-2 hover:bg-white rounded-lg px-2 py-1.5 transition-all duration-200 hover:shadow-sm group">
                <Avatar className="w-8 h-8 border-2 border-gray-200/60 group-hover:border-[#8a25ed]/30 transition-colors">
                  ...
                  <AvatarFallback className="bg-[#8a25ed] text-white text-xs">JD</AvatarFallback>
                </Avatar>
                <div className="hidden lg:block text-left">
                  <p className="text-xs text-gray-900">John Doe</p>
                  <p className="text-xs text-gray-500">user@getcore.id</p>
                </div>
            ...
            <DropdownMenuContent align="end" className="w-56">
              <DropdownMenuLabel className="text-xs">
                ...
                <p className="text-xs text-gray-500 mt-0.5">user@getcore.id</p>
              </DropdownMenuLabel>
